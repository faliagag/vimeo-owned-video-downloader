...
async function appendLog(line) {
  ...
  const next = [`[${new Date().toLocaleTimeString()}] ${line}`, ...list].slice(0, 100);
...
function buildCard(video, idx) {
  ...
  qualField.innerHTML = `<label>Calidad</label><select>
...
function render(embeds) {
  ...
  if (!embeds || !embeds.length) {
    statusEl.textContent = '⚠️ No se detectaron embeds de Vimeo. Asegúrate que la página tenga un iframe de Vimeo cargado y recarga.';
    return;
  }
  statusEl.textContent = `✅ ${embeds.length} embed(s) de Vimeo detectado(s).`;
...
saveHostBtn.addEventListener('click', async () => {
  ...
  hostInfo.textContent = 'Dominio permitido: ' + (h || 'sin configurar');
