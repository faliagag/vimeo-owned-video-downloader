 ...
 * 1. MP4 progresivo / download directo desde playerConfig
 * 2. HLS → descarga de segmentos → blob .ts
 * 3. config.js de Vimeo (fetch público del endpoint)
 * 4. oEmbed API de Vimeo (metadata + thumbnail)
 * Todas sin API key.
 */
'use strict';
...
async function getEmbeds(tabId) {
  ...
  // Esperar un poco para que MutationObserver y timers disparen
...
// ── Estrategia 3: config.js público ───────────────────────────────────────
async function fetchVimeoConfigJs(videoId, referer) {
  // Vimeo expone https://player.vimeo.com/video/{id}/config
  // con o sin parámetros; primero sin hash, luego con el referer como byline
  const urls = [
    `https://player.vimeo.com/video/${videoId}/config`,
    `https://player.vimeo.com/video/${videoId}/config?autopause=1&autoplay=0&loop=0&muted=0&title=0&byline=0&portrait=0`,
  ];
  const headers = {
...
// ── Parsear candidatos del playerConfig ───────────────────────────────────
...
async function downloadSegments(manifestUrl, manifestText, referer, tabId, videoId, title) {
  ...
  for (let j = 0; j < segs.length; j++) {
    ...
    if (j % 5 === 0 || j === segs.length - 1)
      sendProgress('Seg '+(j+1)+'/'+segs.length, Math.round((j/segs.length)*60)+5, tabId, videoId, title);
    let attempts = 4;
    while (attempts-- > 0) {
      try {
  ...
  sendProgress('Ensamblando ('+Math.round(totalBytes/1024/1024)+' MB)…', 68, tabId, videoId, title);
...
async function triggerBlobDownload(uint8array, filename, mime, tabId, videoId, title) {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      sendProgress('Enviando '+Math.round(uint8array.length/1024/1024)+' MB…', 74, tabId, videoId, title);
      await chrome.tabs.sendMessage(dlTabId, { type:'DOWNLOAD_META', filename, mime, totalChunks });
      for (let i = 0; i < totalChunks; i++) {
        const start = i * CHUNK_SIZE;
        const slice = uint8array.slice(start, Math.min(start+CHUNK_SIZE, uint8array.length));
        ...
        sendProgress('Parte '+(i+1)+'/'+totalChunks,
          74 + Math.round(((i+1)/totalChunks)*20), tabId, videoId, title);
      }
      sendProgress('Finalizando…', 95, tabId, videoId, title);
      await new Promise((res2, rej2) => {
        const t2 = setTimeout(() => {
          chrome.runtime.onMessage.removeListener(onDone);
          cleanup();
          rej2(new Error('Timeout confirmación (30s).'));
        }, 30000);
...
async function tryDownload(payload) {
  ...
  // Intento 3: oEmbed (solo metadata, sin archivos directos normalmente)
  let oembed = null;
  if (!config) {
  ...
  if (!config) {
    return {
      ...
      message: '❌ No se pudo obtener playerConfig ni config.js. ' +
        (oembed ? 'oEmbed OK ("'+oembed.title+'" de '+oembed.author_name+') pero sin URL de descarga directa.' :
  ...
  if (direct?.url) {
    try {
      ...
      sendProgress('✅ MP4 directo iniciado.', 100, tabId, vimeoId, title);
      return { ok:true, message:'✅ Descarga MP4 directa iniciada ('+direct.quality+').', direct:true };
    } catch(e) {
      sendProgress('⚠ MP4 directo falló, intentando HLS…', 20, tabId, vimeoId, title);
    }
  }
  ...
  if (hls?.url) {
    return {
      ...
      tabId,
      videoId: vimeoId,
      message: '⏳ Iniciando conversión HLS…'
    };
  }

  return {
    ok: false,
    message: '❌ Config encontrada pero sin archivos descargables. ' +
      'Candidatos: ' + candidates.map(c => c.source+'/'+c.quality).join(', ')
  };
}
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  (async () => {
    ...
    if (type === 'DIAGNOSE_VIDEO') {
      ...
      // diagnose no requiere dominio guardado, solo info
      let cfg = null; let src = 'none';
      try {
        const r = await getPlayerConfig(payload.tabId, payload.vimeoId);
        if (r?.config) { cfg = r.config; src = r.source; }
      } catch(_) {}
      if (!cfg) {
        const tab = await getActiveTab();
        const cfg2 = await fetchVimeoConfigJs(payload.vimeoId, tab?.url);
        if (cfg2) { cfg = cfg2; src = 'config.js'; }
      }
      if (!cfg) {
        const oe = await fetchOEmbed(payload.vimeoId);
        if (oe) return sendResponse({ ok:true, message:'oEmbed: "'+oe.title+'" de '+oe.author_name+' | Sin archivos directos. El video puede ser privado.' });
        return sendResponse({ ok:false, message:'❌ Sin acceso al video '+payload.vimeoId+'. Comprueba que el iframe esté cargado y visible.' });
      }
      const cands = parseCandidates(cfg);
      const direct = pickBestDirect(cands);
      const hls = pickBestHls(cands);
      return sendResponse({ ok:true, message:
        '✅ "'+(cfg.video?.title||'?')+'" | Fuente: '+src+
        ' | MP4: '+(direct?direct.source+' '+direct.quality:'NO')+
        ' | HLS: '+(hls?'SÍ':'NO')+
