/* background.js v8.5
 * + DEBUG_IFRAMES: lista todos los iframes de la página para diagnóstico
 */
'use strict';
...
async function ensureFloater(tabId) {
  ...
  } catch (e) { console.warn('[VDF] ensureFloater:', e.message); }
}
...
async function downloadSegments(manifestUrl, manifestText, referer, tabId, videoId, title) {
  ...
  for (let j = 0; j < segs.length; j++) {
    ...
    if (j % 5 === 0 || j === segs.length - 1) sendProgress('Seg ' + (j + 1) + '/' + segs.length, Math.round((j / segs.length) * 60) + 5, tabId, videoId, title);
    let attempts = 4;
    while (attempts-- > 0) {
      try {
...
async function triggerBlobDownload(uint8array, filename, mime, tabId, videoId, title) {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      sendProgress('Enviando ' + Math.round(uint8array.length / 1024 / 1024) + ' MB en ' + totalChunks + ' parte(s)…', 74, tabId, videoId, title);
      ...
      for (let i = 0; i < totalChunks; i++) {
        ...
        sendProgress('Parte ' + (i + 1) + '/' + totalChunks, 74 + Math.round(((i + 1) / totalChunks) * 20), tabId, videoId, title);
      }
      sendProgress('Disparando descarga…', 95, tabId, videoId, title);
      await new Promise((res2, rej2) => {
        const t2 = setTimeout(() => { chrome.runtime.onMessage.removeListener(onDone); cleanup(); rej2(new Error('Timeout confirmacion (30s).')); }, 30000);
        ...
        chrome.tabs.sendMessage(dlTabId, { type: 'DOWNLOAD_FINALIZE' }).catch(e => { clearTimeout(t2); chrome.runtime.onMessage.removeListener(onDone); cleanup(); rej2(new Error('FINALIZE: ' + e.message)); });
      });
      resolve();
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  (async () => {
    ...
    // NUEVO: diagnóstico de iframes para debug
    ...
    if (type === 'GET_RAW_CONFIG') {
      ...
      return sendResponse({ ok: true, filesKeys: r.config?.request?.files ? Object.keys(r.config.request.files) : [], candidates: cands, videoTitle: r.config?.video?.title || '' });
    }
    if (type === 'TRY_DOWNLOAD' || type === 'DIAGNOSE_VIDEO') {
      ...
      if (type === 'DIAGNOSE_VIDEO') return sendResponse({ ok: true, message: '✅ "' + videoTitle + '" | MP4: ' + (direct ? direct.source + ' ' + direct.quality : 'NO') + ' | HLS: ' + (hls ? 'SÍ' : 'NO') + ' | Candidatos: ' + candidates.length });
      if (direct?.url) {
        try {
      ...
      if (hls?.url) return sendResponse({ ok: true, converting: true, hlsUrl: hls.url, title, pageUrl: payload.pageUrl, tabId: payload.tabId, videoId: payload.vimeoId, message: '⏳ Iniciando HLS…' });
